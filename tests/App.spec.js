...
describe("Authorization", async ({ page }) => {
  ...
  await page.click('[data-testid="login-submit-btn"]');
    ...
    const title = await page.$eval('a:has-text("Моё обучение")', (element) => element.textContent);
    const expectedTitle = "Моё обучение";
    expect(title).toBe(expectedTitle);
  });
  test("Failed Authorization", async ({ page }) => {
    await page.goto("https://netology.ru/?modal=sign_in");
    await page.fill('[placeholder="Email"]', "jsaqa60@pochta.ru");
    await page.fill('[placeholder="Пароль"]', "11111");
    await page.click('[data-testid="login-submit-btn"]');
